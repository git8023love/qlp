...
import com.qlp.cloud.common.security.exception.*;
 ...
 */
@Slf4j
public class QlpWebResponseExceptionTranslator implements WebResponseExceptionTranslator {
    private ThrowableAnalyzer throwableAnalyzer = new DefaultThrowableAnalyzer();

    @Override
    @SneakyThrows
    public ResponseEntity<OAuth2Exception> translate(Exception e) {        // Try to extract a SpringSecurityException from the stacktrace
    ...
    private ResponseEntity<OAuth2Exception> handleOAuth2Exception(OAuth2Exception e) {
        ...
        return new ResponseEntity<>(new QlpAuth2Exception(e.getMessage(), e.getOAuth2ErrorCode()), headers,
                HttpStatus.valueOf(status));
    }
}
